...
libraryRouter.get("/", (req, res) => {
  try {
    const { dir, items } = listLibraryItems();
    const withUrls = items.map((item) => ({
      ...
      pngUrl: item.pngPath ? `/api/library/image/${item.id}` : null,
      hasJson: Boolean(item.jsonPath),
    }));
    return res.json({ ok: true, dir, items: withUrls });
  } catch (e: any) {
...
libraryRouter.post("/save", async (req, res) => {
  try {
    ...
    const { dir, id } = saveLibraryCard(body.card, format, avatarPng);
    return res.json({ ok: true, id, dir });
  } catch (e: any) {
    return res.status(200).json({ ok: false, error: String(e?.message ?? e) });
  }
});
...
libraryRouter.post("/update/:id", async (req, res) => {
  try {
    ...
    const { dir, id } = updateLibraryCard(req.params.id, body.card, format, avatarPng);
    return res.json({ ok: true, id, dir });
  } catch (e: any) {
    return res.status(200).json({ ok: false, error: String(e?.message ?? e) });
  }
...
libraryRouter.get("/:id", (req, res) => {
  try {
    const { cardV2, hasPng } = loadLibraryCard(req.params.id);
    return res.json({
      ok: true,
      cardV2,
      avatarPngUrl: hasPng ? `/api/library/image/${req.params.id}` : null,
    });
  } catch (e: any) {
    return res.status(200).json({ ok: false, error: String(e?.message ?? e) });
  }
...
libraryRouter.get("/image/:id", (req, res) => {
  try {
    const png = loadLibraryPng(req.params.id);
